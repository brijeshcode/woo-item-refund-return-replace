...
<?php
	if (isset($_GET['action']) && $_GET['action'] == 'approve' && isset($_GET['item_id'])) {
		phoe_admin_approve_order_item_request($_GET['item_id'], 'Refund');
	}

	if (isset($_GET['action']) && $_GET['action'] == 'denied' && isset($_GET['item_id'])) {
		phoe_admin_denied_order_item_request($_GET['item_id'], 'Refund');
	}
	$cancelRequests = get_customer_order_item_requests('Refund');
?>
...
<div class="phoe-card-header">
	<div class="row">
		...
		<div class="col-6 text-right"><h2><a href="<?= add_query_arg(['tab' => 'refund-setting']) ?>">Settings</a></h2></div>
	</div>
