...
if (isset($_POST['request_item_action'])) {
	if (in_array($_POST['request_item_action'], ['exchange', 'cancel', 'refund']) ) {
		...
		$message = phoe_create_customer_order_request($_POST['request_item_action']);
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										if (isset($action['id'])) {
											// code...
											echo '<a href="' .$action['url'] . '" class="woocommerce-button button ' . sanitize_html_class( $key ) . '"
											data-toggle="modal" data-target="#'.$action['id'].'"

											onclick="add_request_data(\''.$action['order_id'].'\', \'\', \''.$action['id'].'\' )"
